...
train = pd.read_csv(path+"train.csv", dtype=dtypes,usecols=train_cols, parse_dates=['click_time'])# ,nrows=10000)
test = pd.read_csv(path+"test_supplement.csv", parse_dates=['click_time'],dtype=dtypes, usecols=test_cols)# ,nrows=10000)
...
print('start 2min')
train_recent_2min = get_last_time_click(train_rc,['app', 'channel'],roll_time = '2min')
test_recent_2min = get_last_time_click(test_rc,['app', 'channel'],roll_time = '2min')
data_recent_2min = pd.concat([train_recent_2min,test_recent_2min])
del train_recent_2min, test_recent_2min
data_recent_2min.to_pickle('data/feature_future_ac_2min_data.pkl')
...
print('start 15min')
train_recent_15min = get_last_time_click(train_rc,['app', 'channel'],roll_time = '15min')
test_recent_15min = get_last_time_click(test_rc,['app', 'channel'],roll_time = '15min')
data_recent_15min = pd.concat([train_recent_15min,test_recent_15min])
del train_recent_15min, test_recent_15min
data_recent_15min.to_pickle('data/feature_future_ac_15min_data.pkl')
...
print('start 30min')
train_recent_30min = get_last_time_click(train_rc,['app', 'channel'],roll_time = '30min')
test_recent_30min = get_last_time_click(test_rc,['app', 'channel'],roll_time = '30min')
data_recent_30min = pd.concat([train_recent_30min,test_recent_30min])
del train_recent_30min, test_recent_30min
data_recent_30min.to_pickle('data/feature_future_ac_30min_data.pkl')
...
print('start 60min')
train_recent_60min = get_last_time_click(train_rc,['app', 'channel'],roll_time = '60min')
test_recent_60min = get_last_time_click(test_rc,['app', 'channel'],roll_time = '60min')
data_recent_60min = pd.concat([train_recent_60min,test_recent_60min])
del train_recent_60min, test_recent_60min
data_recent_60min.to_pickle('data/feature_future_ac_60min_data.pkl')
